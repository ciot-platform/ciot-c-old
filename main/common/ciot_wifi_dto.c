...
static ciot_err_t ciot_wifi_scan_status_to_json(CJSON_PARAMETERS(ciot_wifi_scan_status_t));
...
{
    CJSON_CHECK_PARAMETERS();
    CJSON_ADD_OBJ_IN_UNION(sta, ciot_wifi_sta_status_to_json, interface, CIOT_WIFI_IF_STA);
    CJSON_ADD_OBJ_IN_UNION(ap, ciot_wifi_ap_status_to_json, interface, CIOT_WIFI_IF_AP);
    return CIOT_ERR_OK;
}
...
static ciot_err_t ciot_wifi_sta_status_to_json(CJSON_PARAMETERS(ciot_wifi_sta_status_t))
{
    ...
    CJSON_ADD_OBJ(scan, ciot_wifi_scan_status_to_json);
...
static ciot_err_t ciot_wifi_scan_status_to_json(CJSON_PARAMETERS(ciot_wifi_scan_status_t))
{
    ...
    CJSON_ADD_OBJ_PTR(result, ciot_wifi_ap_info_to_json);
    return CIOT_ERR_OK;
}
